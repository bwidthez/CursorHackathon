...
Allow a **landlord** to document a tenant’s property condition by uploading photos, allow the **tenant** to upload their own photos for the same review, and let an **admin** review all photos and issue a **voucher reward** (e.g. cinema) to the tenant when satisfied (thumbs up). This is separate from the existing task/submission flow where landlords approve tenant task submissions.

### Roles

| Role     | Responsibility in this feature                                      |
| -------- | -------------------------------------------------------------------- |
| Landlord | Create a property review and upload photos of the property.          |
| Tenant   | Upload photo(s) for a review tied to their property (optional).      |
...
1. Landlord logs in and goes to a “Property reviews” or “Review property” area.
2. Landlord selects a property (and optionally the tenant, if multiple tenants per property).
3. Landlord creates a new “Property review” (e.g. “Inspection March 2026”).
4. Landlord uploads one or more photos (camera or file picker). Each photo is sent to the backend and stored (e.g. URL or blob reference).
5. Landlord optionally adds a short note and submits. Review status is e.g. **pending_admin_review**.
6. Landlord can see the review in a list with status (pending / approved / rejected).

### 2.2 Tenant: Upload photo(s) for a review

1. Tenant logs in and sees “Property reviews” or “Upload photos for review” for their property.
2. Tenant sees any review(s) created by the landlord for their property that are still open (e.g. pending admin review).
...
| `tenantId`       | ObjectId | Reference to `users` (tenant). |
...
| ------ | ----------------- | ------- | ----------- |
| POST   | `/api/property-reviews` | landlord | Create a property review; body: `propertyId`, `tenantId`, optional `landlordNote`. Returns `propertyReviewId`. |
...
| POST   | `/api/property-reviews/{id}/tenant-photos` | tenant | Tenant uploads photo(s) for this review (multipart). Append to `tenantPhotos`. Tenant may only add to reviews for their property. |
| GET    | `/api/property-reviews` | landlord, tenant, admin | List reviews. Landlord: own reviews; tenant: reviews for their property; admin: all. Query: `propertyId`, `tenantId`, `status`. |
...
- **List:** Table or cards of property reviews (property, tenant, date, status).
- **Create:** “New property review” → select property (and tenant if needed) → optional note → “Add photos” (camera or file) → submit. Show success and link to review detail.
...
- **List:** Reviews for the tenant’s property (pending / approved / rejected).
...
| ---- | ------------------ |
| Number of reviews per property | **No limit.** Landlords may create as many property reviews as needed. |
...
- **Frequency of reviews:** **No limit** on the number of reviews per property.
...
- **Landlord:** Creates property review, uploads photos. No limit on how many reviews can be created.
